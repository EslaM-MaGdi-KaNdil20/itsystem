...
export default function ImportEmployeesModal({ isOpen, onClose, onSuccess }) {
  ...
  const handlePreview = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/employees/import/preview`, {
        method: 'POST',
        body: formData
      });
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/employees/import/execute`, {
        method: 'POST',
        body: formData
      });
        ...
        >
          ...
          <div className="bg-gradient-to-r from-blue-600 to-blue-700 text-white p-6">
            <div className="flex justify-between items-center">
              <div>
                <h2 className="text-xl font-bold">استيراد الموظفين من Excel</h2>
              ...
              <div className="flex items-center gap-2">
                {[1, 2, 3].map((s) => (
                  <div key={s} className="flex items-center">
                    ...
                    {s < 3 && <div className={`w-12 h-1 mx-1 ${step > s ? 'bg-white' : 'bg-blue-500'}`} />}
                  </div>
                ))}
              </div>
            </div>
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      <h3 className="font-semibold text-blue-900">تحميل القالب</h3>
                      <p className="text-sm text-blue-700">قم بتحميل قالب Excel واملأه بالبيانات</p>
                    </div>
                    <button
                      ...
                      <table className="w-full text-sm">
                        <thead className="text-green-700">
                          <tr>
                            ...
                            <th className="text-right p-2">الوظيفة</th>
                          </tr>
                        </thead>
                        <tbody>
                          {previewData.valid.map((emp, i) => (
                            <tr key={i} className="border-t border-green-200">
                              <td className="p-2">{emp.employee_code || '-'}</td>
                  ...
                  <div className="bg-red-50 border border-red-200 rounded-xl p-4">
                    ...
                    <div className="max-h-40 overflow-y-auto">
                      <table className="w-full text-sm">
                        ...
                        <tbody>
                          {previewData.invalid.map((emp, i) => (
                            <tr key={i} className="border-t border-red-200">
                              <td className="p-2">{emp.row}</td>
                              <td className="p-2">{emp.full_name || '-'}</td>
                              <td className="p-2 text-red-600">{emp.errors.join('، ')}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
